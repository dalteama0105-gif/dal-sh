...
const CMS = {
    ...
    async renderOrganization() {
        ...
        const orgChart = document.getElementById('organization-chart');
        if (!orgChart) return;

        orgChart.innerHTML = `
            <div class="org-level">
                <div class="org-card fade-in">
                    <h4>${data.about.organization.ceo.name}</h4>
                    <p>${data.about.organization.ceo.title}</p>
                    <span>${data.about.organization.ceo.department}</span>
                </div>
            </div>
            <div class="org-level">
                <div class="org-children">
                    ${data.about.organization.leadership.map((leader, index) => `
                        <div class="org-card fade-in" style="animation-delay: ${index * 0.1}s">
                            <h4>${leader.name}</h4>
                            <p>${leader.title}</p>
    ...
    async renderTeam() {
        ...
        teamGrid.innerHTML = data.about.team.map((member, index) => `
            <div class="team-member fade-in" style="animation-delay: ${index * 0.1}s">
                <img src="${member.photo}" alt="${member.name}" class="team-photo" onerror="this.style.background='linear-gradient(135deg, #2563eb, #3b82f6)'">
                <h4>${member.name}</h4>
